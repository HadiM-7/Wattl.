"""
Compile captured PNG frames into animated GIFs.
Produces two sizes:
  - profile-picture.gif  (250x250 for Gmail profile)
  - email-signature.gif  (500x500 for email signature)
"""
...
def main():
    ...
    compile_gif(
        frames_250,
        os.path.join(OUTPUT_DIR, "profile-picture.gif"),
        FRAME_INTERVAL,
        max_colors=96,
    )
    ...
    compile_gif(
        frames_500,
        os.path.join(OUTPUT_DIR, "email-signature.gif"),
        FRAME_INTERVAL,
        max_colors=128,
    )

    print("\nDone! GIFs saved to:")
    print(f"  {os.path.join(OUTPUT_DIR, 'profile-picture.gif')}")
    print(f"  {os.path.join(OUTPUT_DIR, 'email-signature.gif')}")
